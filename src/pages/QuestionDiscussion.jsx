...
function QuestionDetails({ question }) {
  return (
    <>
      ...
      <div className="pb-5">
        <p className="my-5">{question.description}</p>
    ...
    <>
      <div className="py-5">
        <h1 className="text-xl mb-3">
          ...
          {answersForQuestion.length == 1 ? "Answer" : "Answers"}
        </h1>
      ...
      <div className="mb-5">
        <div className="mb-3">
          <label htmlFor="answer">
            <h3 className="text-xl my-3">Answer this question</h3>
          </label>
          <FormTextArea
            value={text}
            handler={(e) => {
...
export default function QuestionDiscussion() {
  ...
  useEffect(() => {
    async function getQuestions() {
      const fetchQuestions = await getQuestionById(parseInt(questionId), token);
      setQuestion(fetchQuestions);
      try {
        const fetchAnswers = await getAnswers(parseInt(questionId), token);
